...
//dbWrk.creer(manPers.courantPersonne());
    ...
    private void menuModifier(ActionEvent event) {
        ...
        try {
            ...
            JfxPopup.displayError("Erreur", "", ex.getMessage());
        }
        modeAjout = false;
    }
    ...
    private void sauverPersonne(ActionEvent event) {
        ...
        Personne p = new Personne(Integer.valueOf(txtPK.getText()), txtPrenom.getText(),txtNom.getText(), java.sql.Date.valueOf(dateNaissance.getValue()), Integer.valueOf(txtNo.getText()), txtRue.getText(), Integer.valueOf(txtNPA.getText()), txtLocalite.getText(), Boolean.valueOf(ckbActif.getText()), Double.valueOf(txtSalaire.getText()), new Date());
        if (modeAjout == true) {
            try {
                dbWrk.creer(p);
            } catch (MyDBException ex) {
